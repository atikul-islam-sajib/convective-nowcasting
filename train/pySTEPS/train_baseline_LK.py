...
class Config:
    ...
    out_dir = 'LK_EXTRAPOLATION_PYSTEPS'
    ...
    num_in_frames = 5
    lk_num_frames = 3  # frames used to estimate dense Lucas-Kanade motion
    stride_minutes = 5
    horizons = [15, 30, 45]
...
def run_lk_extrapolation_with_velocity(radar_history, velocity, cfg):
    """
    Deterministic precipitation extrapolation using a Lucas-Kanade velocity field.

    The previous radar frames are used to estimate the velocity field outside this
    function. The actual forecast advects only the latest radar frame forward.
    """
    ...
    for horizon in cfg.metric_horizons:
        ...
        index = step - 1
        predictions[horizon] = np.clip(
            forecast_mm[index], 0.0, CLIP_MAX_MMH
        ).astype(np.float32)

    return predictions

def run_both_baselines(radar_history, satellite_history, channel_stats, cfg):
    print('\n    Preparing Lucas-Kanade extrapolation input...')
...
def main():
    ...
    print('Forecast   : dense Lucas-Kanade motion + deterministic extrapolation')
    
    print(f'LK history frames: {cfg.lk_num_frames} (spanning {(cfg.lk_num_frames - 1) * cfg.stride_minutes} min back)')
    print('=' * 80)
    ...
    if worker_args:
        if cfg.num_processes > 1 and len(worker_args) > 1:
            print(f'\nRunning {len(worker_args)} samples across {cfg.num_processes} worker processes (each sample uses deterministic LK extrapolation)...\n')
