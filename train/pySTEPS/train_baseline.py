...
def process_one_sample(args):
    ...
    for model_name, predictions in [('PySteps_Radar', radar_predictions), ('PySteps_Multimodal', multimodal_predictions)]:
        ...
        for horizon in cfg.metric_horizons:
            ...
            thr_str = ' | '.join(
                f'CSI@{int(t) if float(t).is_integer() else t}={metrics[f"CSI@{int(t) if float(t).is_integer() else t}"]:.4f}'
                for t in cfg.categorical_thresholds
            )
            ets_str = ' | '.join(
                f'ETS@{int(t) if float(t).is_integer() else t}={metrics[f"ETS@{int(t) if float(t).is_integer() else t}"]:.4f}'
                for t in cfg.categorical_thresholds
            )
