...
class MultiHorizonTrainingConfig:
    ...
    checkpoint_dir = 'CHECKPOINTS_CONVLSTM_PATCH_CROSSATTN_1KM_SUMMER_2H_2015_to_2024'
    train_viz_dir  = 'TRAINVIS_CONVLSTM_PATCH_CROSSATTN_1KM_SUMMER_2H_2015_to_2024'
    val_viz_dir    = 'VALVIZ_CONVLSTM_PATCH_CROSSATTN_1KM_SUMMER_2H_2015_to_2024'
    ...
    mlflow_experiment   = 'nowcasting_convlstm_patch_crossattn__2015_to_2024'
    mlflow_run_name     = 'patch256_2H_convlstm_wmae_gradloss_crossattn_2015_to_2024'
...
def mlflow_log_config(cfg, horizons, n_train, n_val, total_params):
    mlflow.log_params({
        ...
        'cross_attn_embed_dim':        cfg.cross_attn_embed_dim,
        'cross_attn_num_heads':        cfg.cross_attn_num_heads,
        'cross_attn_window_size':      cfg.cross_attn_window_size,
...
class WindowedCrossAttentionFusion(nn.Module):
    def __init__(self, radar_dim=1, sat_dim=2, embed_dim=64,
                 num_heads=4, window_size=8, dropout=0.1):
        super().__init__()
        self.embed_dim   = embed_dim
        self.window_size = window_size
        self.num_heads   = num_heads

        self.radar_proj = nn.Conv2d(radar_dim, embed_dim, kernel_size=1, bias=False)
        self.sat_proj   = nn.Conv2d(sat_dim,   embed_dim, kernel_size=1, bias=False)
        ...
        self.out_proj = nn.Conv2d(embed_dim, radar_dim, kernel_size=1, bias=False)
    ...
    def _merge_windows(self, x, B, H, W):
        ws = self.window_size
        C  = x.shape[-1]
        nH, nW = H // ws, W // ws
    ...
    def forward(self, radar, satellite):
        ...
        q_map  = self.radar_proj(radar)
        kv_map = self.sat_proj(satellite)

        q  = self._partition_windows(q_map)
        kv = self._partition_windows(kv_map)
        ...
        fused_map = self._merge_windows(attn_out, B, H, W)

        delta = self.out_proj(fused_map)

        return radar + delta


class ConvLSTMWrapper(nn.Module):
    def __init__(self, num_channels, num_timesteps=5, n_horizons=2, device='cpu',
                 use_cross_attention=False,
                 cross_attn_embed_dim=64, cross_attn_num_heads=4,
                 cross_attn_window_size=8, cross_attn_dropout=0.1):
        super().__init__()
        ...
        if use_cross_attention:
            self.cross_attn_fusion = WindowedCrossAttentionFusion(
                radar_dim   = 1,
                sat_dim     = 2,
    ...
    def forward(self, x):
        ...
        if self.use_cross_attention:
            ...
            for t in range(self.num_timesteps):
                ...
                radar_fused = self.cross_attn_fusion(radar_t, sat_t)

                fused_t = torch.cat([sat_t, radar_fused], dim=1)
                fused_timesteps.append(fused_t)

            x = torch.stack(fused_timesteps, dim=1)
...
def save_visualizations(model, loader, epoch, transform, cfg,
    ...
    attn_tag = " [CrossAttn]" if cfg.use_cross_attention else ""
    fig.suptitle(
        f"Epoch {epoch:03d}  |  {split_name.upper()} -- Best Storm Samples"
        f"  [ConvLSTM{attn_tag}]",
...
def train(model, train_loader, val_loader, cfg, transform,
    ...
    attn_tag = "  [CrossAttn ENABLED]" if cfg.use_cross_attention else "  [CrossAttn DISABLED]"

    print("\n" + "="*80)
    print(f"MULTI-HORIZON TRAINING -- ConvLSTM  [2H: t+15, t+30]{attn_tag}")
    print("="*80)
    ...
    if cfg.use_cross_attention:
        ...
        print(f"    embed_dim     : {cfg.cross_attn_embed_dim}")
        print(f"    num_heads     : {cfg.cross_attn_num_heads}")
        print(f"    window_size   : {cfg.cross_attn_window_size}x{cfg.cross_attn_window_size}"
              f"  ({cfg.cross_attn_window_size}km receptive field)")
        print(f"    dropout       : {cfg.cross_attn_dropout}")
        n_windows = (256 // cfg.cross_attn_window_size) ** 2
        print(f"    windows/frame : {n_windows}  "
              f"({256//cfg.cross_attn_window_size}x{256//cfg.cross_attn_window_size} grid)")
        print(f"    tokens/window : {cfg.cross_attn_window_size**2}")
    else:
        print(f"  Cross-Attention : DISABLED (baseline concatenation)")
    print("="*80 + "\n")
    ...
    if cfg.loss_type == 'hybrid_weighted_mae':
        ...
        print("HybridWeightedMAE [5 thresholds: 3/7/15/25/35mm/h -> 3/8/40/80/120x]\n")
        ...
        print("HybridWeightedMSE [5 thresholds: 3/7/15/25/35mm/h -> 3/8/40/80/120x]\n")
    else:
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print("MULTI-HORIZON TRAINING -- ConvLSTM  [2H: t+15, t+30]")
    print("="*80)
    ...
    print(f"  Cross-Attention : {cfg.use_cross_attention}"
          + (f"  (embed={cfg.cross_attn_embed_dim}, heads={cfg.cross_attn_num_heads},"
             f" window={cfg.cross_attn_window_size}x{cfg.cross_attn_window_size})"
             if cfg.use_cross_attention else ""))
    ...
    print(f"\nBuilding Model... [ConvLSTM + CrossAttn={cfg.use_cross_attention}]")
    ...
    attn_params  = (sum(p.numel() for p in model.cross_attn_fusion.parameters())
                    if cfg.use_cross_attention and hasattr(model, 'cross_attn_fusion')
                    else 0)
    print(f"   Parameters: {total_params:,}"
          + (f"  (cross-attn: {attn_params:,})" if attn_params > 0 else ""))
