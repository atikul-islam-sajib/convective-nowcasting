...
class MultiHorizonTrainingConfig:
    ...
    checkpoint_dir = 'CHECKPOINTS_EARTHFORMER_PATCH_1KM_SUMMER_2H_2015_to_2024_t15_to_t60_PRE_FINAL'
    train_viz_dir  = 'TRAINVIS_EARTHFORMER_PATCH_1KM_SUMMER_2H_2015_to_2024_t15_to_t60_PRE_FINAL'
    val_viz_dir    = 'VALVIZ_EARTHFORMER_PATCH_1KM_SUMMER_2H_2015_to_2024_t15_to_t60_PRE_FINAL'
    ...
    cosine_eta_min = 5e-5 # 5e-5
    cosine_eta_max = 5e-4 # 5e-5
    cosine_T_max   = 30
    cosine_T_0     = 30
    ...
    resume_from = "CHECKPOINTS_EARTHFORMER_PATCH_1KM_SUMMER_2H_2015_to_2024_t15_to_t60_PRE_FINAL/best_ets.pth"

    mlflow_experiment   = 'nowcasting_earthformer_patch_2015_to_2024_t15_to_t60_PRE_FINAL'
    mlflow_run_name     = 'patch256_2H_earthformer_wmae_2015_to_2024_t15_to_t60_PRE_FINAL'
...
def mlflow_log_config(cfg, horizons, n_train, n_val, total_params):
    mlflow.log_params({
        ...
        'n_val_samples':               n_val,
        'model_backbone':              'EarthFormer_CuboidTransformer',
        'earthformer_config_yml':      cfg.earthformer_config_yml,
...
def save_visualizations(model, loader, epoch, transform, cfg,
    ...
    fig.suptitle(
        f"Epoch {epoch:03d}  |  {split_name.upper()} -- Best Storm Samples  [EarthFormer]",
...
def train(model, train_loader, val_loader, cfg, transform,
    ...
    print("\n" + "="*80)
    print("MULTI-HORIZON TRAINING -- EarthFormer  [2H: t+15, t+30]")
    print("="*80)
    ...
    print(f"  Gradient Loss   : DISABLED (not safe for EarthFormer)")
    print(f"  Perceptual Loss : {'ENABLED weight=' + str(cfg.perceptual_weight) if cfg.use_perceptual_loss else 'DISABLED'}")
    print(f"  Mixed Precision : {cfg.use_mixed_precision}  (KEEP FALSE -- AMP causes NaN)")
...
if __name__ == "__main__":
    ...
    print(f"  LR Scheduler       : {cfg.lr_scheduler}")
    print(f"  EarthFormer config : {cfg.earthformer_config_yml}")
    ...
    assert model_cfg['input_shape'] == [n_timesteps, 256, 256, n_channels_per_step], (
        f"earthformer_nowcast.yaml input_shape mismatch!\n"
        f"  config  : {model_cfg['input_shape']}\n"
        f"  expected: [{n_timesteps}, 256, 256, {n_channels_per_step}]"
    )
    assert model_cfg['target_shape'] == [n_horizons, 256, 256, 1], (
        f"earthformer_nowcast.yaml target_shape mismatch!\n"
        f"  config  : {model_cfg['target_shape']}\n"
        f"  expected: [{n_horizons}, 256, 256, 1]"
    )

    print(f"\nBuilding Model... [EarthFormer / CuboidTransformer]")
    ...
    print(f"   target_shape:     {model_cfg['target_shape']}")
    print(f"   base_units:       {model_cfg['base_units']}")
    print(f"   enc_depth:        {model_cfg['enc_depth']}")
    print(f"   dec_depth:        {model_cfg['dec_depth']}")
    print(f"   num_heads:        {model_cfg['num_heads']}")

    model = EarthFormerWrapper(
