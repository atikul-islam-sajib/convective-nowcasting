...
def satellite_path(base, dt, channel):
    return os.path.join(base, dt.strftime('%Y'), dt.strftime('%m'),
                        dt.strftime('%d'), dt.strftime('%H%M%S') + f'_{channel}.npy')
...
def load_full_sample(row, cfg):
    ...
    with open(stats_path, 'r') as f:
        channel_stats = {int(k): v for k, v in json.load(f).items()}
    ...
    for ch in cfg.channels:
        ch_int = int(ch.replace('CH', ''))
        for t in history_times:
            arr = np.load(satellite_path(cfg.sat_base, t, ch)).astype(np.float32)
            if ch_int in channel_stats:
                arr = (arr - channel_stats[ch_int]['mean']) / \
                      (channel_stats[ch_int]['std'] + 1e-8)
            satellite_planes.append(arr)
    ...
    input_np     = np.stack(satellite_planes + radar_planes, axis=0).astype(np.float32)
...
class InferenceConfig:
    ...
    zerovalue_db        = -15.0
    psnr_data_range      = CLIP_MAX_MMH
    csi_threshold_mmh    = 10.0
    categorical_thresholds = [10.0]
...
def main():
    parser = argparse.ArgumentParser(description="Full-image multi-model DL nowcasting comparison + ensemble")
    ...
    parser.add_argument("--csi_threshold_mmh", type=float, default=None)
    ...
    if all_metrics:
        ...
        mean_df["CSI-M"] = mean_df["CSI"]
        mean_df["ETS-M"] = mean_df[[c for c in mean_df.columns if c.startswith("ETS@")]].mean(axis=1)
