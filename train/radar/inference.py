...
def compute_metrics(prediction, target, mask, cfg):
    ...
    # CSI at one or more thresholds -> CSI@<thr> columns
...
def load_all_models(cfg):
    # Radar-only: single input channel (no satellite channels added).
    n_ch  = 1
...
def load_full_sample(row, cfg):
    # Radar-only: no satellite planes, no channel_stats.json needed.
