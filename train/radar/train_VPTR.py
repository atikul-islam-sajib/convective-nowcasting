...
class MultiHorizonTrainingConfig:
    ...
    checkpoint_dir = 'CHECKPOINTS_VPTR_PATCH_1KM_SUMMER_2H_2015_to_2024_t15_to_t60_RADAR'
    train_viz_dir  = 'TRAINVIS_VPTR_PATCH_1KM_SUMMER_2H_2015_to_2024_t15_to_t60_RADAR'
    val_viz_dir    = 'VALVIZ_VPTR_PATCH_1KM_SUMMER_2H_2015_to_2024_t15_to_t60_RADAR'
    ...
    mlflow_experiment   = 'nowcasting_vptr_patch_2015_to_2024_t15_to_t60' #-> :( Not working
    mlflow_run_name     = 'patch256_2H_vptr_wmae_gradloss_2015_to_2024_t15_to_t60' # :( not working
...
def mlflow_log_config(cfg, horizons, n_train, n_val, total_params):
    mlflow.log_params({
        ...
        'vptr_feat_dim':               cfg.vptr_feat_dim,
        'vptr_n_downsampling':         cfg.vptr_n_downsampling,
        'vptr_encH':                   cfg.vptr_encH,
        'vptr_encW':                   cfg.vptr_encW,
        'vptr_d_model':                cfg.vptr_d_model,
        'vptr_nhead':                  cfg.vptr_nhead,
        'vptr_num_encoder_layers':     cfg.vptr_num_encoder_layers,
        'vptr_num_decoder_layers':     cfg.vptr_num_decoder_layers,
        'vptr_dropout':                cfg.vptr_dropout,
        'vptr_window_size':            cfg.vptr_window_size,
        'vptr_spatial_ffn_ratio':      cfg.vptr_spatial_ffn_ratio,
        'vptr_rpe':                    cfg.vptr_rpe,
...
class VPTRWrapper(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = VPTREnc(
            img_channels   = num_channels,
            feat_dim       = feat_dim,
            n_downsampling = n_downsampling,
        )

        self.former = VPTRFormerNAR(
            num_past_frames          = num_timesteps,
        ...
        # ← num_channels → 1: works for both radar-only (1→1) and
        #   satellite+radar (3→1). No change needed here.
        self.out_proj = nn.Conv2d(num_channels, 1, kernel_size=1, bias=True)

    def forward(self, x):
        B, TC, H, W = x.shape
        assert TC == self.num_timesteps * self.num_channels, \
            f"Expected {self.num_timesteps * self.num_channels} channels, got {TC}"
        ...
        pred_flat = pred_frames.view(B2 * T2, C2, H2, W2)
        out_flat  = self.out_proj(pred_flat)
        out       = out_flat.view(B2, T2, H2, W2)
...
def save_visualizations(model, loader, epoch, transform, cfg,
    ...
    fig.suptitle(
        f"Epoch {epoch:03d}  |  {split_name.upper()} -- Best Storm Samples  [VPTR]",
...
class GradientSharpnessLoss(nn.Module):
    ...
    def forward(self, pred, target, mask):
        pred_mm   = inverse_transform_torch(pred.clamp(0.0, 1.0))
        target_mm = inverse_transform_torch(target.clamp(0.0, 1.0))

        dx_pred = (pred_mm[:, :, 1:] - pred_mm[:, :, :-1]).clamp(-50.0, 50.0)
        dy_pred = (pred_mm[:, 1:, :] - pred_mm[:, :-1, :]).clamp(-50.0, 50.0)
        dx_gt   = (target_mm[:, :, 1:] - target_mm[:, :, :-1]).clamp(-50.0, 50.0)
        dy_gt   = (target_mm[:, 1:, :] - target_mm[:, :-1, :]).clamp(-50.0, 50.0)
...
def train_epoch(model, loader, loss_fn, opt, device, grad_clip,
    ...
    for batch_idx, (inputs, targets, masks) in pbar:
        ...
        if scaler is not None:
            ...
            loss.backward()
        t_backward_total += time.time() - _tb
    ...
    print(f"\n  [PROFILER] Data={t_data_total:.1f}s | "
          f"Forward={t_forward_total:.1f}s | "
          f"Backward={t_backward_total:.1f}s"
          + (" <-- IO BOUND, consider more workers or caching"
             if t_data_total > t_forward_total else " <-- GPU BOUND, good"))
...
def train(model, train_loader, val_loader, cfg, transform,
    ...
    print("\n" + "="*80)
    print("MULTI-HORIZON TRAINING -- VPTR  [RADAR ONLY]")
    print("="*80)
    ...
    if cfg.resume_from and os.path.exists(cfg.resume_from):
        ...
        print(f"epochs_without_improvement reset to 0 after resume\n")
    ...
    for epoch in range(start_epoch, cfg.num_epochs + 1):
        ...
        print(f"    Avg ETS@{cfg.ets_threshold_mmh:.0f}mm/h : {mean_ets:.4f}  (across all horizons)")
...
if __name__ == "__main__":
    ...
    print(f"  LR Scheduler  : {cfg.lr_scheduler}")
    print(f"  Input Mode    : RADAR ONLY (satellite channels disabled)")
    print(f"  Batch size    : {cfg.batch_size} x accum {cfg.gradient_accumulation_steps} "
          f"= effective {cfg.batch_size * cfg.gradient_accumulation_steps}")
    print(f"  GPUs          : {torch.cuda.device_count()}x")
    print(f"  VPTR encH/W   : {cfg.vptr_encH}x{cfg.vptr_encW}  "
          f"(n_downsampling={cfg.vptr_n_downsampling})")
    print(f"  VPTR d_model  : {cfg.vptr_d_model}  nhead={cfg.vptr_nhead}  "
          f"layers={cfg.vptr_num_encoder_layers}/{cfg.vptr_num_decoder_layers}")
    print(f"  VPTR window   : {cfg.vptr_window_size}  "
          f"({cfg.vptr_encH}/{cfg.vptr_window_size}="
          f"{cfg.vptr_encH//cfg.vptr_window_size} windows/dim)")
    ...
    train_dataset = SatelliteRadarPatchDataset(
        ...
        mask_nans=policy.mask_nans, validate_files=False, use_cache=True,
        radar_only=True,                                    
    )
    ...
    n_channels_per_step = 1                                
    expected_channels   = n_timesteps * n_channels_per_step

    print(f"\nBuilding Model... [VPTR — RADAR ONLY]")
    print(f"   Input shape:      {sample_input.shape}")
    print(f"   n_timesteps:      {n_timesteps}")
    print(f"   channels/step:    {n_channels_per_step}  (radar only)")
    print(f"   Total channels:   {n_channels} (expected {expected_channels})")
    assert n_channels == expected_channels, \
        f"Channel mismatch: got {n_channels}, expected {expected_channels}"

    assert cfg.vptr_encH % cfg.vptr_window_size == 0, (
        f"vptr_encH={cfg.vptr_encH} must be divisible by "
        f"vptr_window_size={cfg.vptr_window_size}"
    )
    assert cfg.vptr_encW % cfg.vptr_window_size == 0, (
        f"vptr_encW={cfg.vptr_encW} must be divisible by "
        f"vptr_window_size={cfg.vptr_window_size}"
    )

    model = VPTRWrapper(
        num_channels             = n_channels_per_step,
        num_timesteps            = n_timesteps,
        n_horizons               = n_horizons,
        feat_dim                 = cfg.vptr_feat_dim,
        n_downsampling           = cfg.vptr_n_downsampling,
        encH                     = cfg.vptr_encH,
        encW                     = cfg.vptr_encW,
        d_model                  = cfg.vptr_d_model,
        nhead                    = cfg.vptr_nhead,
        num_encoder_layers       = cfg.vptr_num_encoder_layers,
        num_decoder_layers       = cfg.vptr_num_decoder_layers,
        dropout                  = cfg.vptr_dropout,
        window_size              = cfg.vptr_window_size,
        Spatial_FFN_hidden_ratio = cfg.vptr_spatial_ffn_ratio,
        rpe                      = cfg.vptr_rpe,
    )
    ...
    with torch.no_grad():
        _dummy = torch.zeros(2, n_timesteps * n_channels_per_step, 256, 256).to(cfg.device)
        _out   = model(_dummy)
        assert _out.shape == (2, n_horizons, 256, 256), \
            f"Unexpected output shape: {_out.shape}"
        print(f"  Shape check passed: {_out.shape}  ✓")
    del _dummy, _out
    torch.cuda.empty_cache()
