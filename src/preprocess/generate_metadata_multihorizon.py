...
def _process_one_timestamp(args):
    ...
    if not is_valid_sample(
        ...
        satellite_times,
        radar_target_times,   # list of datetimes
...
def generate_metadata_and_split(
    ...
    all_csv   = os.path.join(out_dir, "all_samples_multihorizon.csv")
    train_csv = os.path.join(out_dir, "train_multihorizon.csv")
    val_csv   = os.path.join(out_dir, "val_multihorizon.csv")
    test_csv  = os.path.join(out_dir, "test_multihorizon.csv")

    if os.path.exists(all_csv) and not force:
        print("Multi-horizon metadata already exists – skipping generation")
        return
    ...
    horizons = compute_forecast_horizons(config.temporal.radar_lead_minutes)
    print("=" * 80)
    print("MULTI-HORIZON METADATA GENERATION")
    print("=" * 80)
    ...
    print("=" * 80)
    print("MULTI-HORIZON METADATA GENERATION COMPLETE")
    print("=" * 80)
