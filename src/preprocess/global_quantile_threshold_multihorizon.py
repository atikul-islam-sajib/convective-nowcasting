...
def compute_global_threshold_chunked(csv_path, percentile, n_jobs, sample_size=10_000_000):
    ...
    Supports multi-horizon CSV with columns: radar_path_15/30/45/60
    """
    df = pd.read_csv(csv_path)

    if "split" in df.columns:
        df = df[df["split"] == "train"]
        print(f"Using only 'train' split: {len(df)} rows")

    # ── Multi-horizon path extraction ──────────────────────────
    radar_cols = ['radar_path_15', 'radar_path_30', 'radar_path_45', 'radar_path_60']
    available  = [c for c in radar_cols if c in df.columns]

    if len(available) == 0:
        # fallback: original single-column format
        if 'radar_path' in df.columns:
            paths = df['radar_path'].dropna().unique()
            print(f"Using single column 'radar_path': {len(paths):,} files")
        else:
            raise ValueError(
                f"No radar path column found. Available columns: {list(df.columns)}"
            )
    else:
        paths = pd.concat([df[c] for c in available]).dropna().unique()
        print(f"Using columns: {available}")
        print(f"Total unique radar files: {len(paths):,}")
    ...
    for batch_idx in range(num_batches):
        ...
        batch_pixels = Parallel(n_jobs=n_jobs)(
            delayed(load_nonzero_pixels)(p)
            for p in tqdm(batch_paths, desc=f"Batch {batch_idx+1}")
        )
...
def main():
    ...
    info = {
        ...
        "sampling_method":   "chunked_ultra_defensive_multihorizon",
        "note":              "Computed from radar_path_15/30/45/60 columns, unique files only"
    }
    ...
    print(f"P{args.percentile} THRESHOLD = {thr:.4f} mm/h")
